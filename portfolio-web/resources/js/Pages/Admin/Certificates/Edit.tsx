...
export default function CertificateEdit({ certificate }: CertificateEditProps) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        focus: certificate.focus || '',
        link: certificate.link || '#',
        image: certificate.image || '',
        image_file: null as File | null,
        order: certificate.order ?? 0,
        is_published: certificate.is_published ?? true,
    });
    ...
    return (
        <AdminLayout
            ...
            subtitle="Update certificate information and credential links."
            actions={
                <Link
            ...
            <form onSubmit={submit} className="space-y-8 max-w-4xl">
                <div className="rounded-2xl border border-slate-200/80 bg-white/70 p-6 shadow-sm backdrop-blur-md dark:border-slate-800/80 dark:bg-slate-900/50 space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4 border-t border-slate-100 dark:border-slate-800">
                        ...
                        <div className="flex items-center gap-3 pt-6">
                            <input
                                type="checkbox"
                                id="is_published_cert_edit"
                                checked={data.is_published}
                                onChange={(e) => setData('is_published', e.target.checked)}
                                className="h-4 w-4 rounded border-slate-300 text-blue-600"
                            />
                            <label htmlFor="is_published_cert_edit" className="text-sm font-semibold text-slate-800 dark:text-slate-200 cursor-pointer">
                ...
                <div className="flex justify-end gap-3">
                    ...
                    >
                        ...
                        <span>{processing ? 'Saving...' : 'Update Certificate'}</span>
                    </button>
                </div>
            </form>
        </AdminLayout>
